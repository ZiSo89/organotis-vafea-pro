...
class APIService {
    ...
    async handleOnlineRequest(table, action, data, id) {
        ...
        if (action === 'list') {
            ...
            endpoint += `&id=${id}`;
            // Use POST with _method for hosts that don't support PUT
            options = { method: 'POST', body: JSON.stringify({ ...data, _method: 'PUT' }) };
            result = await this.request(endpoint, options);
        } else if (action === 'delete' && id) {
            endpoint += `&id=${id}`;
            // Use POST with _method for hosts that don't support DELETE
            options = { method: 'POST', body: JSON.stringify({ id, _method: 'DELETE' }) };
            result = await this.request(endpoint, options);
        }
